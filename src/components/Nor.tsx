  ...
  return (
    <div className="w-full bg-white font-sans text-[#1a1a1a] overflow-x-hidden mt-10 md:mt-20">
      ...
      <div className="max-w-[1400px] mx-auto px-4 md:px-10">
        <h1 className="text-[36px] sm:text-[50px] md:text-[68px] font-black text-[#1a1a1a] mb-6 md:mb-10 tracking-[-0.04em] leading-tight md:leading-none uppercase">
          Նորություններ
        </h1>
        ...
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-6 mb-10 md:mb-16">
          ...
          <button className="flex items-center gap-3 bg-[#f3e8ff] text-[#730D8D] px-6 md:px-8 py-2.5 md:py-3 rounded-full text-[12px] md:text-[14px] font-black hover:bg-[#ebd5ff] transition-colors uppercase tracking-wider">
        ...
        <div className="flex flex-col lg:flex-row items-center gap-8 md:gap-12 mb-16 md:mb-24 group cursor-pointer">
          <div className="flex-1 order-2 lg:order-1 w-full">
             <div className="flex items-center gap-3 mb-4 md:mb-6">
               ...
               <span className="text-[#1a1a1a] font-black text-[10px] md:text-[12px] uppercase tracking-[0.1em]">Նորարարություններ</span>
             </div>
             <h2 className="text-[26px] sm:text-[34px] md:text-[48px] font-black leading-[1.1] mb-4 md:mb-6 tracking-[-0.03em] group-hover:text-[#730D8D] transition-colors">
               {newsItems[0].title}
             </h2>
             <p className="text-[#4a4a4a] text-[16px] md:text-[18px] leading-[1.6] max-w-2xl font-medium mb-4">
               Կցեք Ձեր Visa կամ Mastercard քարտերը Google Wallet-ին և կատարեք անհպում վճարումներ Ձեր Android սարքով:
             </p>
             <p className="text-gray-400 font-bold text-[12px] md:text-[14px]">{newsItems[0].date}</p>
          </div>
          
          <div className="flex-1 order-1 lg:order-2 relative w-full">
            <div className="absolute -top-2 -left-2 md:-top-4 md:-left-4 w-8 md:w-12 h-8 md:h-12 border-t-[6px] md:border-t-[8px] border-l-[6px] md:border-l-[8px] border-[#730D8D] z-10 transition-transform group-hover:-translate-x-1 group-hover:-translate-y-1"></div>
            ...
            <div className="absolute -bottom-2 -left-2 md:-bottom-4 md:-left-4 w-8 md:w-12 h-8 md:h-12 border-b-[6px] md:border-b-[8px] border-l-[6px] md:border-l-[8px] border-[#730D8D] z-10 transition-transform group-hover:-translate-x-1 group-hover:translate-y-1"></div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-x-6 gap-y-10 md:gap-y-16 mb-16 md:mb-24">
          {newsItems.slice(1).map((item, idx) => (
            <div key={idx} className="group cursor-pointer flex flex-col">
              <div className="overflow-hidden mb-4 md:mb-5 rounded-sm relative aspect-[1.33]">
                <img 
                  src={item.img} 
                  alt={item.title}
              ...
              <div className="flex items-center gap-2 mb-2 md:mb-3">
                ...
                <span className="text-[#1a1a1a] font-black text-[10px] md:text-[11px] uppercase tracking-[0.1em]">{item.category}</span>
              </div>
              <h3 className="font-bold text-[16px] md:text-[18px] leading-[1.3] tracking-tight group-hover:text-[#730D8D] transition-colors flex-grow">
                {item.title}
              </h3>
              <p className="text-gray-400 font-bold text-[12px] md:text-[13px] mt-3 md:mt-4 uppercase tracking-tighter">{item.date}</p>
            </div>
          ))}
        </div>
        ...
        <div className="flex items-center gap-4 text-gray-400 font-black text-[14px] md:text-[16px] pb-16 md:pb-24 border-t border-gray-100 pt-8 md:pt-10">
           ...
           <span className="text-[#730D8D] text-[20px] md:text-[24px] font-black cursor-pointer ml-4 transition-transform hover:translate-x-2">→</span>
        </div>
